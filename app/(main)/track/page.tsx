...
import { fetchTrackingData, fetchDirections } from "@/lib/services/orders"
import {
  getStatusLabel,
...
function TrackingContent() {
  ...
  React.useEffect(() => {
    async function loadTrackingData() {
      if (!trackingNumber || !session?.accessToken || !session?.refreshToken) {
        setIsLoading(false)
        if (!trackingNumber) {
      ...
      setError(null)
      try {
        // Fetch tracking data directly using the tracking number
    ...
    loadTrackingData()
  }, [trackingNumber, session?.accessToken, session?.refreshToken, updateSession, router])
